...
label50 = LabelFrame(tk,height=120,width=150,text="电机状态",labelanchor='n',bg="whitesmoke")#labelanchor属性解决定位问题（n:北，nw:西北，center:正中，其余类推）
...
button41 = Button(tk, textvariable=Switch_mode, width=25,command=Start_serial_launcher)#button启动的函数不能接受参数，此时使用lambda，先计算函数结果，然后把结果强制重组为一个匿名简单函数
...
button61 = Button(tk, textvariable=searching_status, width=25,command=Searching_port_launcher)#button启动的函数不能接受参数，此时使用lambda，先计算函数结果，然后把结果强制重组为一个匿名简单函数
