...
class Desk:
    ...
    def set_corner_dict(self):  # 设置角点字典
        ...
        showinfo("角点设置提示", "请按照右下-左下-左上-右上的顺序双击左键你要变换区域的四个顶点")
...
class Paddle:
    def __init__(self, frame=None):
        ...
        self.lower_blue = np.array([50, 43, 46])  # 绿色阈值的下限
        self.upper_blue = np.array([70, 255, 255])  # 绿色阈值的上限
    ...
    def locate(self):  # 球定位,生成外接矩形的各个参数
        try:
            ...
            (self.center_x, self.center_y), self.radius = cv.minEnclosingCircle(contours[index])
        except:
            pass
    
    def draw_locate(self, frame):  # 绘制外接矩形,生成frame_locate
        try:
            self.frame_locate = frame.copy()
            center = (int(self.x), int(self.y))
            radius = int(self.radius)
            self.frame_locate = cv2.circle(self.frame, center, radius, (0, 255, 0), 2)
        except:
            self.frame_locate = frame.copy()
            pass
    ...
    def draw_tracking(self, frame):  # 根据运动学参数生成预测运动轨迹,生成frame_track
        try:
            ...
            cv.line(self.frame_track, (self.center_x, self.center_y),
                    (self.center_x + 100 * self.uint_x, self.center_y + 100 * self.uint_y),
                    (255, 0, 0), 5)
        except:
            self.frame_track = frame.copy()
            pass
...
class Ball:
    def __init__(self, frame=None):
        ...
        self.x = 0  # 目标质心的x坐标测量
        self.y = 0  # 目标质心的y坐标测量
        self.kx=0#卡尔曼滤波目标质心的x坐标测量
        self.ky=0#卡尔曼滤波目标质心的y坐标测量
        self.vx = 0  # 目标运动的x速度
        self.vy = 0  # 目标运动的y速度
        self.uint_x = 0  # 目标运动单位方向的x分量
        self.uint_y = 0  # 目标运动单位方向的y分量
        self.kernel_morphologyEx_size = 4  # 开运算核
        self.kernel_dilate_size = 3  # 膨胀运算核
        ...
        try:
            ...
            self.frame_preprocess = cv.dilate(cv.morphologyEx(self.frame_thresh,
                    ...
                    (self.kernel_morphologyEx_size, self.kernel_morphologyEx_size), np.uint8)),
                                              np.ones((self.kernel_dilate_size, self.kernel_dilate_size), np.uint8),
                                              iterations=1)  # 先进行开运算降噪后进行膨胀复原
            self.frame_preprocess=cv.morphologyEx(self.frame_preprocess, cv.MORPH_CLOSE, np.ones(
                    (self.kernel_morphologyEx_size, self.kernel_morphologyEx_size), np.uint8))
        except:
            showerror("错误！", "发生未知错误,请检查!")
    ...
    def tracking_analysis(self,frame, time):  # 运动分析，生成运动学各个参数,输入差帧和时间
        ...
        self.vx = (self.x - frame.x) / time
        self.vy = (self.y - frame.y) / time
...
def Cam_Setting(desk, ball,paddle):
        ...
        def show_track():  # 显示添加追踪定位标记后的图像
            ...
            try:
                while (True):
                    ...
                    ball.locate()
                    ball_before.locate()
                    ball.draw_locate(ball.frame_original)
    ...
    def paddle_set_gui():
        ...
        hmin = StringVar(root, value=str(paddle.lower_blue[0]))
        hmax = StringVar(root, value=str(paddle.upper_blue[0]))
        smin = StringVar(root, value=str(paddle.lower_blue[1]))
        smax = StringVar(root, value=str(paddle.upper_blue[1]))
        vmin = StringVar(root, value=str(paddle.lower_blue[2]))
        vmax = StringVar(root, value=str(paddle.upper_blue[2]))
        kernel1 = StringVar(root, value=str(paddle.kernel_morphologyEx_size))
        kernel2 = StringVar(root, value=str(paddle.kernel_dilate_size))
        sleep_time = StringVar(root, value=str(paddle.time))
        ...
        label0 = Label(root, text="设置阈值范围(默认黄色)：", bg="whitesmoke")
        label1 = Label(root, text="Hmin", bg="whitesmoke")
        label2 = Label(root, text="Smin", bg="whitesmoke")
        label3 = Label(root, text="Vmin", bg="whitesmoke")
        label4 = Label(root, text="Hmax", bg="whitesmoke")
        ...
        def show_original():  # 显示原图像
            ...
            try:
                while (True):
                    desk.get_frame()
                    paddle.reflesh(desk.frame)
                    cv.imshow('camera original', paddle.frame_original)
                    if cv.waitKey(1) & 0xff == ord('q'):
                        cv.destroyWindow('camera original')
                        desk.capture.release()
                        break
        ...
        def show_segmentation():  # 显示分割后的图像
            showinfo("提示", "退出请按键盘q键,播放过程请勿随意点击菜单界面")
            cv.startWindowThread()
            paddle.lower_blue = np.array([int(hmin.get()), int(smin.get()), int(vmin.get())])
            paddle.upper_blue = np.array([int(hmax.get()), int(smax.get()), int(vmax.get())])
            desk.set_capture()
            try:
                while (True):
                    desk.get_frame()
                    paddle.reflesh(desk.frame)
                    paddle.preprocess(paddle.frame_original)
                    cv.imshow('camera segmentation', paddle.frame_segmentation)
                    if cv.waitKey(1) & 0xff == ord('q'):
                        cv.destroyWindow('camera segmentation')
                        desk.capture.release()
                        break
        ...
        def show_thresh():  # 显示二值图像
            ...
            try:
                while (True):
                    ...
                    paddle.reflesh(desk.frame)
                    paddle.preprocess(paddle.frame_original)
                    cv.imshow('camera thresh', paddle.frame_thresh)
                    if cv.waitKey(1) & 0xff == ord('q'):
                        cv.destroyWindow('camera thresh')
                        desk.capture.release()
                        break
        ...
        def show_reduce_noise():  # 显示降噪后的图像
            ...
            paddle.lower_blue = np.array([int(hmin.get()), int(smin.get()), int(vmin.get())])
            paddle.upper_blue = np.array([int(hmax.get()), int(smax.get()), int(vmax.get())])
            paddle.kernel_morphologyEx_size = int(Kernel1.get())
            paddle.kernel_dilate_size = int(Kernel2.get())
            try:
                while (True):
                    desk.get_frame()
                    paddle.reflesh(desk.frame)
                    paddle.preprocess(paddle.frame_original)
                    cv.imshow('camera reduce noise', paddle.frame_preprocess)
                    if cv.waitKey(1) & 0xff == ord('q'):
                        cv.destroyWindow('camera reduce noise')
                        desk.capture.release()
                        break
        ...
        def show_track():  # 显示添加追踪定位标记后的图像
            ...
            paddle.kernel_morphologyEx_size = int(Kernel1.get())
            paddle.kernel_dilate_size = int(Kernel2.get())
            paddle.time = float(sleep_time.get())
            desk.set_capture()
            try:
                while (True):
                    desk.get_frame()
                    ...
                    paddle.preprocess(paddle.frame_original)
                    paddle.locate()
                    paddle.draw_locate_rectangle(paddle.frame_original)
                    cv.imshow('camera track', paddle.frame_locate)
                    if cv.waitKey(1) & 0xff == ord('q'):
                        cv.destroyWindow('camera track')
                        desk.capture.release()
                        break
        ...
        def load_setting():  # 从文件导入设置
            ...
            s = f.readline()
            s = s.split('|')
            paddle.lower_blue = np.array([int(s[0]), int(s[1]), int(s[2])])
            paddle.upper_blue = np.array([int(s[3]), int(s[4]), int(s[5])])
            paddle.kernel_morphologyEx_size = int(s[6])
            paddle.kernel_dilate_size = int(s[7])
            paddle.time = float(s[8])
            f.close()
            hmin.set(str(paddle.lower_blue[0]))
            smin.set(str(paddle.lower_blue[1]))
            vmin.set(str(paddle.lower_blue[2]))
            hmax.set(str(paddle.upper_blue[0]))
            smax.set(str(paddle.upper_blue[1]))
            vmax.set(str(paddle.upper_blue[2]))
            kernel1.set(str(paddle.kernel_morphologyEx_size))
            kernel2.set(str(paddle.kernel_dilate_size))
            sleep_time.set(paddle.time)
            showinfo("提示", "参数读档成功！")
            root.update()
    ...
    def show_process():
        ...
        try:
            while (True):
                ...
                print('ball.vx=',ball.vx,'ball.vy=',ball.vy)
                if cv.waitKey(1) & 0xff == ord('q'):
                    cv.destroyWindow('camera')
                    desk.capture.release()
                    break
