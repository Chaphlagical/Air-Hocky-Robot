...
class Ball:
    ...
    def preprocess(self, frame,mode):  # 预处理,利用输入的帧生成frame_thresh,frame_segmentation,frame_preprocess
        try:
            ...
            self.frame_preprocess=cv.morphologyEx(self.frame_thresh,cv.MORPH_OPEN, np.ones((self.kernel_open_size, self.kernel_open_size), np.uint8))#开运算
            self.frame_preprocess = cv.morphologyEx(self.frame_preprocess, cv.MORPH_CLOSE, np.ones((self.kernel_close_size, self.kernel_close_size), np.uint8))#闭运算
            '''轮廓检测'''
